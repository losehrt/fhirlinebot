...
RSpec.describe LineConfig, type: :model do
  describe '.channel_id' do
    ...
    context 'when environment variable is not set but database config exists' do
      ...
      let!(:config) { create(:line_configuration, organization: org, channel_id: 'DB_CHANNEL_ID', is_default: true) }
  ...
  describe '.channel_secret' do
    ...
    context 'when database config exists' do
      let(:org) { create(:organization) }
      let!(:config) { create(:line_configuration, organization: org, channel_secret: 'DB_SECRET', is_default: true) }
  ...
  describe '.config' do
    let(:org) { create(:organization) }
    let!(:config) { create(:line_configuration, organization: org, channel_id: 'TEST_CHANNEL', channel_secret: 'TEST_SECRET', redirect_uri: 'https://test.com/callback', is_default: true) }
    ...
    it 'includes organization-specific configuration when organization_id is provided' do
      ...
      expect(result[:channel_id]).to eq('TEST_CHANNEL')
      expect(result[:channel_secret]).to eq('TEST_SECRET')
    end
  end
  ...
  describe 'caching behavior' do
    ...
    it 'caches configuration values' do
      ...
      # Update in database
      config.update(channel_id: 'NEW_CHANNEL')
